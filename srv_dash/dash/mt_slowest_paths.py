...
class SlowestPathsCountMetric(BaseMetric):
    ...
    def metric_title(self):
        return 'slowest server time paths (most used {} paths)'.format(self.max_different_path())

    def sum_query(self, app=None):
        if not app:
            return 'SELECT SUM("mean") from {} GROUP BY "path"'.format(self.name)
        return 'SELECT SUM("mean") from {} WHERE "app" = \'{}\' GROUP BY "path"'.format(self.name, app)
